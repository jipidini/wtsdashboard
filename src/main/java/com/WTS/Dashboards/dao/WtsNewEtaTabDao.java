...
*/	
  ...
  public void newEtaCalculationForChilds(int parentId,int childId , int processId, boolean mainpageNav) throws ParseException {
	  System.out.println("start time loop entered");
	 ...
	 System.out.println("in start DIFF");
		 Iterator<WtsAppMappingTab> apItr=apps.iterator();
		  while(apItr.hasNext()){
			  ...
			  if(nextAppSeq>currentSeq) {
				  WtsNewEtaTab et=getTdyETATxnByParentChildID(nextApp.getParentId(), nextApp.getChildId(),nextApp.getProcessId(),
		...
		//update parent ETA
		  WtsNewEtaTab exist_proc_eta=getTdyETATxnByParentId(parentId,processId, TreatmentDate.getInstance().getTreatmentDate());
		  if(exist_proc_eta==null) {
			  exist_proc_eta= new WtsNewEtaTab();
		  }
...
public WtsNewEtaTab getTdyETATxnByParentChildID(int parentId,int childId,int processid, String treatDt)
{
  ...
  public List<WtsNewEtaTab> getAllChildEta(int parentId, int processId) {
		...
		return (List<WtsNewEtaTab>) entityManager.createQuery(hql).setParameter(1, TreatmentDate.getInstance().getTreatmentDate()).setParameter(2, processId).setParameter(3, parentId).getResultList();
	}
